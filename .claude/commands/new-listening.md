...
`Albert` `Fred` のように読み上げ教材に向かない声である。**この3つ以外は使わない。**

事前チェックが何を守り、何を守らないかを取り違えないこと:

- `say -v '?'` に**無い**名前を書いた場合は、再生時に必ず失敗する。
...
- `say -v '?'` に**有る**が上の3つではない名前(`Albert` `Fred` など)は、
  **事前チェックを通り、そのまま再生できてしまう。** `validate_listening.py` も `voice` の
  実在を確かめない。つまりここでの選び間違いを機械で捕まえる仕組みは無く、
  検証もビルドも通ったまま、聞き取りにくい音声の教材が出来上がる
