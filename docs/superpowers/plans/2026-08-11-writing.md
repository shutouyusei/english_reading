...
async function initWritingList() {
  ...
  try {
    ...
    container.innerHTML =
      `<p class="error">一覧を読み込めませんでした(${escapeHtmlText(err.message)})。</p>`;
    return;
  }
...
function promptCardHtml(meta) {
  ...
  return `
    <div class="card">
      <div class="card-main">
        <h3>${escapeHtmlText(meta.title)}</h3>
...
// docs/js/textmatch.js の escapeHtml はリーディング用の読み込み経路にしか無いため、
// ライティング側は自前で持つ。共有エンジンには手を入れない方針のため。
...
- [ ] **Step 5: Extend the smoke test timeout**
...
```bash
git add app/ui/writing.html app/ui/js/writing-list.js app/ui/index.html \
...
**仕様からの意図的な逸脱:** 仕様書では `escapeHtml` を共有エンジンから借りる想定だったが、`docs/js/textmatch.js` はリーディングの読み込み経路にしか無い。ライティング側で自前の `esc` / `escapeHtmlText` を持つ(共有エンジンに手を入れない方針を優先)。`writing-list.js` と `writing-editor.js` で同じ関数を2回定義しているが、両者は別ページで同時に読み込まれることがなく、共通化のために新しい共有ファイルを作るほどの量ではない。
